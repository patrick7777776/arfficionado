defmodule CastTest do
  ...
  test "cast" do
    ...
    assert cast(
             {:raw_instance, ["1", "1.23", "yes", "Blah blah blah!", "2019-08-28T11:23:18Z"], 1,
              nil},
             @attributes,
  ...
  test "cast missing" do
    assert cast(
             {:raw_instance, [:missing, :missing, :missing, :missing, :missing], 7, nil},
             @attributes,
             &Arfficionado.custom_date_parse/2
  ...
  test "cast custom date format -- no custom parsing function given" do
    assert cast(
             {:raw_instance, ["20190502"], 1, nil},
             [{:attribute, "funky_date", {:date, "yyyymmdd"}, nil}],
             &Arfficionado.custom_date_parse/2
           ) ==
             {:error,
              "Attribute funky_date / yyyymmdd: Please pass in a function for parsing non-iso_8601 dates."}
  end
  ...
  test "cast custom date format -- custom parsing function given" do
    assert cast(
             {:raw_instance, ["20190502"], 1, nil},
             [{:attribute, "funky_date", {:date, "yyyymmdd"}, nil}],
             fn _, _ -> DateTime.from_unix(1_234_567_890) |> elem(1) end
           ) == {:instance, [DateTime.from_unix(1_234_567_890) |> elem(1)], 1, nil}
  end

  test "cast custom date format -- custom parsing function returning error" do
    assert cast(
             {:raw_instance, ["not_a_date"], 1, nil},
             [{:attribute, "funky_date", {:date, "yyyymmdd"}, nil}],
             fn _, _ -> {:error, "Could not parse: not_a_date"} end
